from Scenarios.OptimizationModels.OptModelsFor_DomainPriorityTest.multiobjectiveoptimization import *
...
#set what solution we want
model.setParam(GRB.Param.SolutionNumber, 1)

#Assignment of optimization values
for t in range(T):
    conversionSchedule_set.append(conversionSchedule[t].Xn * p2g_nom)

    storageSchedule_set.append(storageSchedule[t].Xn*p2g_sto_nom)

    pumpSchedule_set.append(pumpSchedule[t].Xn * pump_nom)

    chpDH_Schedule_set.append(chpDH_Schedule[t].Xn)

    gasSupplySchedule_set.append(gasSupplySchedule[t].Xn*gs_nom)

    waterSupplySchedule_set.append(waterSupplySchedule[t].Xn*ws_nom)

    boilerSchedule3_set.append(boilerSchedule3[t].Xn )
    boilerSchedule4_set.append(boilerSchedule4[t].Xn)
    boilerSchedule5_set.append(boilerSchedule5[t].Xn)
    boilerSchedule6_set.append(boilerSchedule6[t].Xn)
    boilerSchedule7_set.append(boilerSchedule7[t].Xn)
    boilerSchedule8_set.append(boilerSchedule8[t].Xn)
    boilerSchedule9_set.append(boilerSchedule9[t].Xn)
    boilerSchedule10_set.append(boilerSchedule10[t].Xn)
    boilerSchedule11_set.append(boilerSchedule11[t].Xn)
    boilerSchedule12_set.append(boilerSchedule12[t].Xn)

    hpSchedule9_set.append(hpSchedule9[t].Xn)
    hpSchedule10_set.append(hpSchedule10[t].Xn)
    hpSchedule11_set.append(hpSchedule11[t].Xn)
    hpSchedule12_set.append(hpSchedule12[t].Xn)

    batterySchedule9_set.append(batterySchedule9[t].Xn*bat_nom)
    batterySchedule10_set.append(batterySchedule10[t].Xn*bat_nom)
    batterySchedule11_set.append(batterySchedule11[t].Xn*bat_nom)
    batterySchedule12_set.append(batterySchedule12[t].Xn*bat_nom)
...
wb.save('Results\MOOptimization.xlsx')
print("Results are available in 'Results' folder", "'MOOptimization.xlsx' file")
