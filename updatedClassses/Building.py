...
class Building(object):
    ...
    def __init__(self, environment,DHConnection=False,labels=None):
        """
        Workflow
        --------
        ...
        DHConnection: boolean
            Specifies buildings to have district heating connection
        """
        self._kind = "building"
        self.labels=labels
        ...
        self.hasDHConnection = DHConnection
    ...
    def get_waterDemand_profile(self,current_values=False):
        """
        Returns water demand curve
        ...
        water_demand_curve : array-like
            Water demand curve in ???
        """
        #  Initialize array with zeros
        water_demand_curve = np.zeros(len(self.apartments[0].demandWater.
            get_power(currentValues=current_values)))

        # Get power curves of each apartment
        for apartment in self.apartments:

            water_demand_curve += apartment.demandWater.get_power(
                    currentValues=current_values)*134/3000000/3600

        return water_demand_curve
